    ...
    <div x-data="{ sidebarOpen: true, mobileSidebar: false }" class="flex min-h-screen">
        ...
        >
            ...
            <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
                @php
                    $badges = \App\Support\AdminSidebarBadges::counts();
                    ...
                    $nav = [
                        ...
                        ['route' => 'admin.users.index', 'label' => 'Users', 'badge' => 'users', 'icon' => 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z'],
                        ['route' => 'admin.products.index', 'label' => 'Products', 'badge' => 'products', 'icon' => 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10'],
                        ['route' => 'admin.orders.index', 'label' => 'Orders', 'badge' => 'orders', 'icon' => 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01'],
                        ['route' => 'admin.seller-applications.index', 'label' => 'Creator Applications', 'badge' => 'creator-applications', 'icon' => 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z'],
                        ['route' => 'admin.payouts.index', 'label' => 'Payouts', 'badge' => 'payouts', 'icon' => 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z'],
                        ...
                        ['route' => 'admin.reviews.index', 'label' => 'Reviews', 'badge' => 'reviews', 'icon' => 'M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z'],
                        ['route' => 'admin.reports.index', 'label' => 'Reports', 'badge' => 'reports', 'icon' => 'M3 21v-4m0 0V5a2 2 0 012-2h6.5l1 1H21l-3 6 3 6h-8.5l-1-1H5a2 2 0 00-2 2z'],
                        ...
                        ['route' => 'admin.messages.index', 'label' => 'Messages', 'badge' => 'messages', 'icon' => 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z'],
                ...
                @foreach ($nav as $item)
                    ...
                    >
                        ...
                        <span x-show="sidebarOpen" x-cloak class="flex min-w-0 flex-1 items-center justify-between gap-2">
                            <span>{{ $item['label'] }}</span>
                            @if (isset($item['badge']) && ($badges[$item['badge']] ?? 0) > 0)
                                <span class="rounded-full bg-yellow-400 px-2 py-0.5 text-xs font-bold leading-none text-yellow-950">{{ $badges[$item['badge']] }}</span>
                            @endif
                        </span>
                    </a>
                @endforeach
