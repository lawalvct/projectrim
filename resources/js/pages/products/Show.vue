...
const settings = computed(
    () => (page.props.settings as Record<string, string>) || {},
);
const activeTab = ref<
    'abstract' | 'toc' | 'chapter1' | 'reviews' | 'messenger'
>('abstract');
...
const shareUrl = computed(() =>
    typeof window !== 'undefined'
        ? `${window.location.origin}/products/${props.product.slug}`
        : '',
);
const shareText = computed(
    () =>
        `Check out "${props.product.title}" on ${settings.value.site_name || 'ProjectRim'}`,
);
...
// --- Smart Link ---
const smartLinkEnabled = computed(
    () => settings.value.smart_link_enabled === '1',
);
...
function switchTab(
    tab: 'abstract' | 'toc' | 'chapter1' | 'reviews' | 'messenger',
) {
    if (tab !== 'abstract') {
        triggerSmartLink();
    }
...
async function sendMessage() {
    ...
    try {
        const { data } = await axios.post(
            `/products/${props.product.id}/messages`,
            msgForm.value,
        );
    ...
    <PublicLayout>
        <div class="container mx-auto px-4 py-8">
            ...
            <nav class="mb-6 text-sm text-muted-foreground">
                ...
                <Link
                    v-if="product.faculty"
                    :href="`/faculty/${product.faculty.slug}`"
                    class="hover:text-primary"
                    >{{ product.faculty.name }}</Link
                >
                <span class="mx-2">/</span>
            ...
            <div class="grid gap-8 lg:grid-cols-3">
                ...
                <div class="space-y-6 lg:col-span-2">
                    ...
                    <div>
                        <div
                            class="relative aspect-[16/10] overflow-hidden rounded-lg bg-muted"
                        >
                            <video
                                ...
                                :poster="
                                    product.images.length
                                        ? `/storage/${product.images[activeImage].path}`
                                        : '/storage/products/images/projectrim_cover_page.png'
                                "
                            ...
                            <button
                                v-if="readyPreviewVideo"
                                type="button"
                                class="absolute right-3 bottom-3 rounded-md bg-black/70 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-black/85"
                    ...
                    <Dialog v-model:open="videoOpen">
                        <DialogContent class="overflow-hidden p-0 sm:max-w-4xl">
                            ...
                            <div class="px-6 pb-6">
                                <video
                                    v-if="videoOpen && readyPreviewVideo"
                                    controls
                                    preload="metadata"
                    ...
                    <div>
                        ...
                        >
                            <span
                                >by
                                <strong>{{ product.user.name }}</strong></span
                            >
                            <span v-if="product.faculty">{{
                                product.faculty.name
                            }}</span>
                            <span v-if="product.department"
                                >/ {{ product.department.name }}</span
                            >
                        </div>
                        <div
                    ...
                    <div>
                        ...
                        <div class="mt-4">
                            <div
                                v-if="
                                    activeTab === 'abstract' && product.abstract
                                "
                            ...
                            <div
                                v-else-if="
                                    activeTab === 'toc' &&
                                    product.table_of_content
                                "
                                class="prose prose-sm max-w-none"
                                v-html="product.table_of_content"
                            />
                            <div
                                v-else-if="
                                    activeTab === 'chapter1' &&
                            ...
                            <div v-else-if="activeTab === 'reviews'">
                                ...
                                >
                                    ...
                                    >
                                        You may review this product more than
                                        once, up to 5 times per hour.
                                    </p>
                                    <div
                                        v-if="reviewSuccess"
                                        role="status"
                                        class="mb-3 rounded-md border border-green-200 bg-green-50 px-3 py-2 text-sm text-green-700 dark:border-green-900 dark:bg-green-950/40 dark:text-green-300"
                                    >
                                        {{ reviewSuccess }}
                                    </div>
                                    <div
                                ...
                                <div
                                    v-else-if="!auth?.user"
                                    class="mb-6 rounded-lg border border-dashed p-4 text-center text-sm text-muted-foreground"
                                >
                                    <Link
                                        href="/login"
                                        ...
                                        @click.prevent="
                                            redirectGuestToLogin(
                                                'Please log in to Review Project.',
                                            )
                                        "
                                        >Sign in</Link
                                    >
                            ...
                            <div v-else-if="activeTab === 'messenger'">
                                <div
                                    v-if="messageSent"
                                    role="status"
                                    class="mb-4 rounded-md border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700 dark:border-green-900 dark:bg-green-950/40 dark:text-green-300"
                                >
                                    <p class="font-medium">
                                        {{ messageSuccess }}
                                    </p>
                                    <p class="mt-1 text-xs">
                                        The message is now available in the
                                        author’s inbox.
                                    </p>
                                    <button
                                        type="button"
                                        class="mt-2 text-xs font-semibold underline underline-offset-2"
                                        @click="prepareAnotherMessage"
                                    >
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        <Textarea
                                            ...
                                            rows="5"
                                            required
                                            placeholder="Write your message to the author(s)..."
                                        />
                                    </div>
                                    <Button
                                        type="submit"
                ...
                <div class="space-y-6">
                    ...
                    <Card>
                        <CardContent class="p-6">
                            ...
                            <Button
                                variant="outline"
                                class="mt-2 w-full gap-2 text-destructive hover:text-destructive"
                            ...
                            <Dialog v-model:open="reportOpen">
                                <DialogContent class="sm:max-w-md">
                                    <DialogHeader>
                                        <DialogTitle
                                            >Report Project</DialogTitle
                                        >
                                        <DialogDescription
                                            >Describe your complaint about this
                                            project. Your name and email will be
                                            included
                                    ...
                                    >
                                        Report submitted successfully. Our team
                        ...
                        <CardContent class="p-6">
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    class="flex h-10 w-10 items-center justify-center rounded-full bg-[#0088cc] text-white transition-opacity hover:opacity-80"
                                ...
                                >
                                    ...
                                    >
                                        <path
                                            d="M15.75 2.25c.36 2.2 1.58 3.63 3.75 3.77v3.58a7.12 7.12 0 0 1-3.7-.9v6.85a6.2 6.2 0 1 1-6.2-6.2c.29 0 .58.02.86.06v3.68a2.6 2.6 0 1 0 1.65 2.42V2.25h3.64z"
                                        />
                                    </svg>
                                </button>
                                <a
                                ...
                                <button
                                    type="button"
                                    class="flex h-10 w-10 items-center justify-center rounded-full bg-muted text-foreground transition-opacity hover:opacity-80"
                                    :title="
                                        copiedLink
                                            ? 'Link copied!'
                                            : 'Copy link'
                    ...
                    <Card>
                        <CardContent class="space-y-3 p-6">
                            ...
                            <dl class="space-y-2 text-sm">
                                ...
                                <div
                                    v-if="product.date_available"
                                    class="flex justify-between"
                                >
                                    <dt class="text-muted-foreground">Date</dt>
                                    <dd>
                                        {{ formatDate(product.date_available) }}
                                    </dd>
                                </div>
                                <div
                                ...
                                >
                                    ...
                                    <dd>
                                        {{
                                            product.files[0].file_type.toUpperCase()
                                        }}
                                        ({{
                                            formatSize(
                                                product.files[0].file_size,
                                            )
                                        }})
                                    </dd>
                                </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <CardContent class="p-4">
                                <h3
                                    class="line-clamp-2 text-sm font-semibold group-hover:text-primary"
                                >
                                    {{ rp.title }}
                                </h3>
                                ...
                                >
                                    <Badge v-if="rp.is_paid" variant="secondary"
                                        >{{ settings.currency_symbol || '$'
                                        }}{{ rp.price }}</Badge
                                    >
                                    <Badge
